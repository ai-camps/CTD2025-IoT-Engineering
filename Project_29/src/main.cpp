...
#include <ArduinoJson.h>      // Include ArduinoJSON library for JSON serialization
...
bool isThingsBoardConnected = false;                         // Flag to track ThingsBoard connection status

// Create a JSON document to hold the data to be sent to ThingsBoard
DynamicJsonDocument jsonDoc(512); // Create a JSON document to hold the data to be sent to ThingsBoard
...
void sendDataToThingsBoard(float temperature_C, float temperature_F, float humidity_RH);          // Send data to ThingsBoard

constexpr auto THINGSBOARD_SEND_INTERVAL = 10000UL; // 10 seconds
unsigned long lastThingsBoardSend = 0;              // Timer for sending data to ThingsBoard
...
void loop()
{
    ...
    // Send data to ThingsBoard every 10 seconds
    if (millis() - lastThingsBoardSend >= THINGSBOARD_SEND_INTERVAL)
    {
        sendDataToThingsBoard(lastTemperatureC, lastTemperatureF, lastHumidity);
...
void sendDataToThingsBoard(float temperature_C, float temperature_F, float humidity_RH)
{
    if (!isThingsBoardConnected)
    {
        jsonDoc.clear();
        Serial.println("Not connected to ThingsBoard, attempting to reconnect...");
        connectToThingsBoard(thingsboardServer, thingsboardPort, accessToken, clientId);
        if (!tb.connected())
        {
    ...
    jsonDoc["deviceModel"] = "ESP32";
    jsonDoc["owner"] = "Jun";
    jsonDoc["group"] = "Instructor";
    jsonDoc["device_id"] = deviceID;
    jsonDoc["temperature_C"] = temperature_C;
    jsonDoc["temperature_F"] = temperature_F;
    jsonDoc["humidity_RH"] = humidity_RH;
    jsonDoc["fan_status"] = fanSpeed;
    jsonDoc["tilt_status"] = lastTiltState;
    jsonDoc["pir_status"] = lastPirState;
    ...
    Serial.println("Calculated JSON buffer size: " + String(jsonSize) + " bytes");
