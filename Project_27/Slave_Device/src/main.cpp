...
void IRAM_ATTR triggerPIR()
{
    ...
    // Control blue LED based on PIR state (local indication)
    if (pirState == HIGH)
    {
        // PIR sensor detected motion - turn on blue LED
        digitalWrite(BLUE_LED_PIN, HIGH);
        // Send PIR_STATUS_ON message to master
        ESPNOW_COMMAND pirMsg;
        pirMsg.commandType = PIR_STATUS_ON;
        pirMsg.fanState = false; // Not used for PIR
        esp_now_send(remoteMacAddress, (uint8_t *)&pirMsg, sizeof(pirMsg));
    }
    else
    {
        // PIR sensor no motion detected - turn off blue LED
        digitalWrite(BLUE_LED_PIN, LOW);
        // Send PIR_STATUS_OFF message to master
        ESPNOW_COMMAND pirMsg;
        pirMsg.commandType = PIR_STATUS_OFF;
        pirMsg.fanState = false; // Not used for PIR
        esp_now_send(remoteMacAddress, (uint8_t *)&pirMsg, sizeof(pirMsg));
    }
}
