...
void handleEspNowRequestReceived(const uint8_t *mac, const uint8_t *incomingData, int len)
{
    if (len == sizeof(ESPNOW_COMMAND))
    {
        ...
        case TILT_STATUS_ON:
            ...
            isRedLEDOn = true;
            Serial.println("Tilt sensor triggered: Red LED ON");
            break;
        case TILT_STATUS_OFF:
            digitalWrite(RED_LED_PIN, LOW);
            isRedLEDOn = false;
            Serial.println("Tilt sensor released: Red LED OFF");
            break;
        case PIR_STATUS_ON:
            digitalWrite(BLUE_LED_PIN, HIGH);
            isBlueLEDOn = true;
            Serial.println("PIR sensor triggered: Blue LED ON");
            break;
        case PIR_STATUS_OFF:
            digitalWrite(BLUE_LED_PIN, LOW);
            isBlueLEDOn = false;
            Serial.println("PIR sensor cleared: Blue LED OFF");
            break;
        default:
            break;
        }
