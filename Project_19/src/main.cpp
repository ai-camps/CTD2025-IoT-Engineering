...
constexpr auto PWM_FREQUENCY = 5000; // PWM frequency in Hz
constexpr auto PWM_RESOLUTION = 8;   // PWM resolution (8-bit = 0-255)
constexpr auto PWM_CHANNEL = 0;      // PWM channel for red LED
constexpr auto BLINK_INTERVAL = 500; // Blink interval in milliseconds
constexpr auto PWM_DUTY_CYCLE = 128; // PWM duty cycle for blinking (50% brightness)
...
void setup()
{
    ...
    ledcWrite(PWM_CHANNEL, 0); // Turn off red LED via PWM
...
void setFan()
{
    ...
    if (isTemperatureTooHigh)
    {
        digitalWrite(FAN_PIN, HIGH); // Fan for high temperature
    }
    else
    {
        digitalWrite(FAN_PIN, LOW); // Fan for low temperature
    }
}
...
void updateRedLEDBlink()
{
    if (isRedLEDBlinking && millis() - lastBlinkTime >= BLINK_INTERVAL)
    {
        ...
        if (isRedLEDOn)
        {
            ledcWrite(PWM_CHANNEL, PWM_DUTY_CYCLE); // Turn on red LED with PWM
        }
        else
        {
            ledcWrite(PWM_CHANNEL, 0); // Turn off red LED
        }
